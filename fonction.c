    				...
    				if (fichier != NULL)
    				{
    					...
    					if (e==1)
    					{
    						...
    						if ((*victory)==1)
               					...
               					if ((*victory)==1)
               					{
               						stage3(ecran,continuer,intro,effet,effet2,musique,mute,posvolb,sonspell,dying,hit,victory);
               					}
    					}
    					else if (e==2)
    					{
    					...
    					{
    						stage3s(ecran,continuer,intro,effet,effet2,musique,mute,posvolb,sonspell,dying,hit,victory);
    					}
    				}	            		
}
...
poscancel.x=800;
poscancel.y=350;
int boucle1=1,quit=0;
int x=0,y=0,couleur1=0,couleur2=0,couleur3=0,tmp=0,postmp[37];
SDL_Event event;
FILE* fichier = NULL;
FILE* fichier2 = NULL;
postmp[0]=*poshero;
...
postmp[35]=cmpsc;
	...
	menu=IMG_Load("image/stage/returnmenu.png");
	menuu=IMG_Load("image/stage/returnmenuu.png");
	confirmation=IMG_Load("image/stage/back1.png");
Confirm=confirm;
Cancel=cancel;
Menu=menu;
Surface=pause;
while (boucle1)
{
	while (SDL_PollEvent(&event))
		...
		if (hover(posconfirm, x, y))
		{
			Confirm=confirm2;
			couleur1=1;
			if (event.button.button == SDL_BUTTON_LEFT && quit==0)
			{
   			...
   			else if (event.button.button == SDL_BUTTON_LEFT && quit==1)
   			{
   				quit=0;
   				(*boucle)=0;
   			}
	...
	SDL_BlitSurface(Surface,NULL,ecran,&pospause);
...
poscancel.y=350;
int boucle1=1;
int x=0,y=0,couleur1=0,couleur2=0,tmp=0,postmp[40],quit=0,couleur3=0;
SDL_Event event;
FILE* fichier = NULL;
FILE* fichier2 = NULL;
postmp[0]=*poshero;
...
postmp[40]=*posninjaa;
	...
	menu=IMG_Load("image/stage/returnmenu2.png");
	menuu=IMG_Load("image/stage/returnmenu22.png");
	confirmation=IMG_Load("image/stage/back2.png");
Confirm=confirm;
Cancel=cancel;
Menu=menu;
Surface=pause;
...
void Pause3(SDL_Surface *ecran,Sint16 *poshero,Sint16 *posmagicien,Sint16 *posenigme1,Sint16 *poshpup,Sint16 *posback,Sint16 *pospolice,SDL_Rect *pose1,SDL_Rect *pose2,SDL_Rect *pose3 ,SDL_Rect *pose4 ,SDL_Rect *pose5 ,SDL_Rect *pose6 ,SDL_Rect *pose7 ,Sint16 *posrock1,Sint16 *postrap1,Sint16 *posterre3,Sint16 *posterre1, Sint16 *posterre2, Sint16 *posterre4, Sint16 *posterre5, Sint16 *posterre6, Sint16 *posterre7, Sint16 *posterre8, Sint16 *posterre9, int *camera,int *continuer, int tsec, int tmin, int hp, int cmpsc ,Sint16 *posninja, SDL_Rect *pose8,Sint16 *poshpup1,Sint16 *posninjaa, int *boucle)
