...
int main ()
{
    ...
    int continuer=1,transp=0,compteurTrans=1,son=0,direction=1,direction2=1,direction3=1,mute=0,temps=0,victory=2;
    SDL_Event event;
    ...
    while (continuer)
    {
        while (SDL_PollEvent(&event))
            switch(event.type)
            {
                ...
                break;
             	case SDL_KEYDOWN:
               		buttons1(ecran,event,effet,effet2,&continuer,musique,&mute,&posvolb,musiquetuto,sonspell,dying,hit,&victory);
                break;
            	case SDL_MOUSEBUTTONUP:
			buttons(ecran,event,effet,effet2,&continuer,musique,&mute,&posvolb,musiquetuto,sonspell,dying,hit,&victory);
                break;
