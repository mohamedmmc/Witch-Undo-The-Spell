...
* @file stage1.c
* @brief test stage1
* @author Melek Chtourou && M'sahel Hbib
* @version 0.5
* @date Apr 08, 2019
*
*
...
while ((boucle)&&((*continuer)))
		...
		if(keystates[SDLK_k] || received==3)
		{
            			if (a==0)
            			{	
            				posans=poshero.y;
		...
		if(keystates[SDLK_d] || received==1)
		{	
			        e=1;
            			animation(&heroactu,herod,&i);
				poshero.x+=speed;
		}
		if(keystates[SDLK_q] || received==2)
		{	
            			e=2;          			
...
//stage1
void stage1(SDL_Surface *ecran,int *continuer,Mix_Music *intro,Mix_Chunk *effet,Mix_Chunk *effet2,Mix_Music *musique,int *mute,SDL_Rect *posvolb,Mix_Chunk *sonspell,Mix_Chunk *dying,Mix_Chunk *hit,int *victory)
{
	(*victory)=2;
