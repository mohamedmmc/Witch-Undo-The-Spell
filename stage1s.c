...
#define speed  6
//stage1
void stage1s(SDL_Surface *ecran,int *continuer,Mix_Music *intro,Mix_Chunk *effet,Mix_Chunk *effet2,Mix_Music *musique,int *mute,SDL_Rect *posvolb,Mix_Chunk *sonspell,Mix_Chunk *dying,Mix_Chunk *hit,int *victory)
{
	...
	*idlel=NULL,*petithero=NULL,*petitmagicien=NULL,*load=NULL,*petithero2=NULL,*petitherog=NULL,*idler=NULL,*barre=NULL,*terre1=NULL,*terre2=NULL,*terre3=NULL,*jumpr=NULL,*jumpl=NULL,*surfacesecondes=NULL,*surfaceminutes=NULL
	,*imgtime=NULL,*enigmestage1=NULL;
	SDL_Rect posback,poshero,pospause,posload,posconfirm,poscancel,pospetithero,posmagicien,posbarre,posmagicienactu,posspell,posspellen,framed[8],frame2[8],frame3[8],pospolice,posportal,posgo,pose1,posscoreS,pose2,pose3,pose4,pose5,pose6,pose7,posenemie1,posenemie2,posenemie3,posenemie4,posenemie5,posenemie6,posenemie7,poshp,posrock1,posvie,posscore,poshpup,posminutes,possecondes,posnoir,
	...
	posload.x=350;
	posload.y=150;
	load=IMG_Load("image/stage/load1.png");
	...
	while ((boucle)&&((*continuer)))
        ...
        scoreint=TTF_RenderText_Blended(fonts,scr, couleur);
       		...
       		switch(event.type)
        	{
				...
				case SDLK_ESCAPE:							Pause1(ecran,&poshero.x,&posmagicien.x,&posenigme1.x,&poshpup.x,&posback.x,&pospolice.x,&posenemie1,&posenemie2,&posenemie3,&posenemie4,&posenemie5,&posenemie6,&posenemie7,&posrock1.x,&postrap1.x,&posterre3.x,&posterre1.x,&posterre2.x,&posterre4.x,&posterre5.x,&posterre6.x,&posterre7.x,&posterre8.x,&posterre9.x,&camera,continuer,tsec,tmin,hp,cmpsc,&boucle);            		break;
		...
		if (poshero.x + poshero.w < posmagicien.x && poshero.x + poshero.w > posmagicien.x-300 && enig==0 )
		{
			...
			enigme3(&ecran, &continuer, &intro, &effet, &effet2, &musique, &mute, &posvolb,&dying,victory);
			SDL_FillRect(ecran, NULL, SDL_MapRGB(ecran->format, 0, 0, 0));
			boucle=0;
		}
