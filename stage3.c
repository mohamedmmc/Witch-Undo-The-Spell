...
void stagefinale(SDL_Surface *ecran,int *continuer,Mix_Chunk *dying,Mix_Music *intro,Mix_Chunk *effet,Mix_Chunk *effet2,Mix_Music *musique,int *mute,SDL_Rect *posvolb,Mix_Chunk *sonspell,int *hp,int *cmpsc ,int *victory)
{
	...
	SDL_Surface *background=NULL,*viewitch=NULL,*witchhurt=NULL,*hurt=NULL,*healthwitch=NULL,*herod=NULL,*herog=NULL,*heroactu=NULL,*score=NULL,*health=NULL,*vie=NULL,*scoreint=NULL,*attackr=NULL,*attackl=NULL,*witch=NULL,*witchl=NULL,*witcha=NULL,*idler=NULL,*idlel=NULL,*spell=NULL,*spellwitch=NULL;
	SDL_Rect poshealthwitch,posback,poshero,framed[8],frame2[8],posspell,poswitch,posspellwitch,poshp,posvie,posviewitch,posscore,posscoreint,postime,posminutes,possecondes;
	int boucle=1,heroattack=0,attack=0,i=0,a=0,posans=460,k=-1,y=0,x=0,j=0,directionhero=1,intspell=0,nbrspell=0,nbrspellwitch=0,intspellwitch=0,compteurwitch=0,witchattack=0,annimationspellwitch=0,hpwitch=20;
	SDL_Event event;
	...
	background=IMG_Load ("image/stage/stagefinal.png");
	...
	witchl=IMG_Load ("image/enemies/witchwalkleft.png");
	witcha=IMG_Load ("image/enemies/witchattack.png");
	witchhurt=IMG_Load ("image/enemies/witchhurtl.png");
	spellwitch=IMG_Load("image/decoration/spellenemie.png");
	score=IMG_Load ("image/decoration/score.png");
	health=IMG_Load("image/health/heart.png");
	healthwitch=IMG_Load("image/health/heart2.png");
...
while ((boucle)&&((*continuer)))
        ...
        sprintf(chhpw, "%d",hpwitch);
        viewitch= TTF_RenderText_Blended(fonts,chhpw, couleur);
        sprintf(scr,"%d",(*cmpsc));
		...
		if(keystates[SDLK_k]&&heroattack==0)
		{
            			if (a==0)
            			{	
            				posans=poshero.y;
		...
		//animation(&witch,witchl,&compteurwitch);
		...
		if(witchattack==1&&annimationspellwitch==0)
		{
		//printf("bbbbb\n");
		        animation(&witch,witcha,&compteurwitch);
		        if(compteurwitch==31)
		        {
		                posspellwitch.x=poswitch.x-poswitch.h/2;
		                posspellwitch.y=poswitch.y+30;
		                intspellwitch=1;
		                annimationspellwitch=1;
		        }
		}
		if(attack>=100)
		{
		...
		//collision de spell avec witch
		if(collision(posspell,poswitch)==1)
		{
		(*cmpsc)+=10;
		posspell.y=2000;		
		intspell=0;
		...
		//collision de spellwitch avec hero
		if(collision(posspellwitch,poshero)==1)
		{
		if((*cmpsc)>0)
		(*cmpsc)-=5;
		nbrspellwitch=0;
		...
		if((*hp)==0)
            		...
            		heroactu=hurt;
            			for (j=0; j<57; j++)
            			{
            			        SDL_BlitSurface(background, NULL, ecran, &posback);
       		                        ...
       		                        SDL_BlitSurface(witch, &framed[j/6], ecran, &poswitch);
            		...
            		heroactu=idler;
            			...
            			{
            			        SDL_BlitSurface(background, NULL, ecran, &posback);
       		                        SDL_BlitSurface(heroactu, &framed[j/6], ecran, &poshero);
       		                        SDL_BlitSurface(witch, &frame2[j/6], ecran, &poswitch);
       		                        SDL_BlitSurface(vie, NULL, ecran, &posvie);	
		                        SDL_BlitSurface(health, NULL, ecran, &poshp);
		                        SDL_BlitSurface(score, NULL, ecran, &posscore);	
		                        SDL_BlitSurface(scoreint, NULL, ecran, &posscoreint);		
		...
		if(nbrspell==28||(collision(posspell,poswitch)==1))
		{
		intspell=0;
		nbrspell=0;
		}
		//annimation spell de witch
		if(intspellwitch==1)
       		{
       		        SDL_BlitSurface(spellwitch,NULL, ecran, &posspellwitch);
       		        posspellwitch.x-=25;
       		        nbrspellwitch++;
       		...
       		if(nbrspellwitch==25)
       		{
       	 	...
       	 	SDL_BlitSurface(witch, &framed[compteurwitch/4], ecran, &poswitch);
       	 	SDL_BlitSurface(healthwitch, NULL, ecran, &poshealthwitch);
       	 	SDL_BlitSurface(health, NULL, ecran, &poshp);
	        SDL_BlitSurface(score, NULL, ecran, &posscore);	
	        SDL_BlitSurface(vie, NULL, ecran, &posvie);
	        SDL_BlitSurface(viewitch, NULL, ecran, &posviewitch);
       		SDL_BlitSurface(scoreint, NULL, ecran, &posscoreint);
   	    	SDL_Flip(ecran);
    	}
...
int stage=0;
	...
	stagefinale(ecran,continuer,dying,intro,effet,effet2,musique,mute,posvolb,sonspell,&hp,&cmpsc,victory);
	...
	while ((boucle)&&((*continuer)))
        ...
        scoreint=TTF_RenderText_Blended(fonts,scr, couleur);
       		...
       		switch(event.type)
        	{
				...
				break;
case SDLK_ESCAPE:
Pause3(ecran,&poshero.x,&posmagicien.x,&posenigme1.x,&poshpup.x,&posback.x,&pospolice.x,&posenemie1,&posenemie2,&posenemie3,&posenemie4,&posenemie5,&posenemie6,&posenemie7,&posrock1.x,&postrap1.x,&posterre3.x,&posterre1.x,&posterre2.x,&posterre4.x,&posterre5.x,&posterre6.x,&posterre7.x,&posterre8.x,&posterre9.x,&camera,continuer,tsec,tmin,hp,cmpsc,&posninja.x,&pose8,&poshpup1.x,&posninjaa.x,victory);
				break;
				case SDLK_m:
		...
		//dying
			...
			posscore.x=120;
			posscore.y=200;
            			gameover=TTF_RenderText_Blended(fonts, "YOU COMPLETED THE GAME !", couleur1);
            			Blit(noir,ecran,posnoir.x,posnoir.y);
 				Blit(gameover,ecran,posgo.x,posgo.y);
 				SDL_Flip(ecran);
